...
object ParquetMergeSchema {
  def main(args: Array[String]): Unit = {
    ...
    val studentsWithNameAge = Array(("leo", 23), ("jack", 21))
    val studentsWithNameAgeDF = sc.parallelize(studentsWithNameAge, 2).toDF("name", "age")
    studentsWithNameAgeDF.write.format("parquet").mode(SaveMode.Append).save("hdfs://hsdoop120:9000/spark-study/students")
    ...
    studentsWithNameGradeDF.write.format("parquet").mode(SaveMode.Append).save("hdfs://hadoop120:9000/spark-study/students")
    ...
    val students = sqlContext.read.option("mergeSchema", "true").parquet("hdfs://spark1:9000/spark-study/students")
